...
function renderForm($id, $firstname, $lastname, $error)
{
    ...
    <form action="" method="post">
        ...
        <div>
            ...
            <strong>Moodule Name: *</strong> <input type="text" name="module_name" value="<?php echo $module_name; ?>"/><br/>
...
if (isset($_POST['submit'])) {
    ...
    if (is_numeric($_POST['id'])) {
        ...
        if ($username == '' || $module_name == '') {
            ...
            //error, display form
            renderForm($id, $username, $module_name, $error);
        } else {
            // save the data to the database
            mysql_query("UPDATE modules SET name='$username', module_name='$module_name' WHERE module_id='$id'")
            or die(mysql_error());
...
} else // if the form hasn't been submitted, get the data from the db and display the form
{
    ...
    if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
        // query db
        $id = $_GET['id'];
        $result = mysql_query("SELECT * FROM modules WHERE module_id=$id")
        or die(mysql_error());
